function unorm = calcUnormLiq(Pa,T,fprops)
...
% T = Temperature(°C)
    ...
    for Ix = 1:numel(Pa)
        ...
        Tvect = interpn(fprops.liquid.unorm, fprops.p*10, fprops.liquid.T-273.15, fprops.liquid.unorm, Pa(Ix));
        % Limit values
        Tl = max([T(Ix),Tvect(1)],[],2);
        Tl = min([Tl,Tvect(end)],[],2);
        % Calculate normalised internal energy
        unorm(Ix) = interp1(Tvect, fprops.liquid.unorm, Tl);
    end
